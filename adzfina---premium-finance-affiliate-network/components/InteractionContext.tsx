    ...
    <InteractionContext.Provider value={{ showToast, openAuthModal, scrollToSection }}>
      ...
      <div className="fixed bottom-6 right-6 z-[60] flex flex-col gap-3 pointer-events-none">
        {toasts.map((toast) => (
          ...
          >
            {toast.type === 'success' ? <CheckCircle size={20} className="text-lime-400" /> : <Info size={20} className="text-blue-400" />}
      ...
      {isAuthModalOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-3xl p-8 w-full max-w-md relative z-10 shadow-2xl animate-fade-in-up">
            ...
            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); showToast("Access granted! Redirecting...", "success"); closeAuthModal(); }}>
              <div>
                <label className="block text-xs font-bold text-zinc-700 uppercase mb-1">Email Address</label>
                <input type="email" required className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 text-zinc-900 focus:outline-none focus:ring-2 focus:ring-lime-400 transition-all" placeholder="you@example.com" />
              </div>
              <div>
                <label className="block text-xs font-bold text-zinc-700 uppercase mb-1">Password</label>
                <input type="password" required className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 text-zinc-900 focus:outline-none focus:ring-2 focus:ring-lime-400 transition-all" placeholder="••••••••" />
              </div>
              <button type="submit" className="w-full bg-zinc-900 text-white font-bold py-4 rounded-full hover:bg-zinc-800 transition-all shadow-lg">
