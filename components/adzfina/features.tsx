...
import { Coins, BarChart2, CreditCard, Users, Link2, Rocket, Wallet, Bitcoin, Landmark } from "lucide-react"
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="bg-zinc-50 rounded-[2.5rem] p-8 md:p-12 border border-zinc-100 relative overflow-hidden group flex flex-col">
            ...
            <div className="relative z-10 mt-4 flex-1">
              ...
              <p className="text-zinc-500 leading-relaxed mb-8 max-w-sm">
                Get paid faster with our diverse withdrawal options. Choose from Crypto, Wire Transfer, PayPal, and
                more.
              </p>
              <div className="flex gap-2 mb-8 flex-wrap">
                {["Crypto", "Wire Transfer", "PayPal"].map((method) => (
                  <span
                    ...
                    onClick={() =>
                      setSelectedPayment(
                        method === "Crypto" ? "crypto" : method === "Wire Transfer" ? "bank" : "paypal"
                      )
                    }
                    className={`px-3 py-1.5 rounded-full text-xs font-bold border shadow-sm flex items-center gap-1 cursor-pointer transition-colors ${
                      (selectedPayment === "crypto" && method === "Crypto") ||
                      (selectedPayment === "bank" && method === "Wire Transfer") ||
                      (selectedPayment === "paypal" && method === "PayPal")
              ...
              <div className="space-y-3">
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center shadow-sm border ${selectedPayment === "crypto" ? "bg-white text-lime-600 border-lime-100" : "bg-zinc-50 text-zinc-500 border-zinc-100"}`}
                    >
                      <Bitcoin size={16} />
                    </div>
                    <span className={`font-bold text-sm ${selectedPayment === "crypto" ? "text-zinc-900" : "text-zinc-600"}`}>
                      USDT / Bitcoin
                    </span>
                  </div>
                  <div
                    className={`w-4 h-4 rounded-full border-2 ${selectedPayment === "crypto" ? "border-lime-500 bg-lime-500" : "border-zinc-200"}`}
                  ></div>
                </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center border ${selectedPayment === "bank" ? "bg-white text-lime-600 border-lime-100 shadow-sm" : "bg-zinc-50 text-zinc-500 border-zinc-100"}`}
                    >
                      <Landmark size={16} />
                    </div>
                    <span className={`font-bold text-sm ${selectedPayment === "bank" ? "text-zinc-900" : "text-zinc-600"}`}>
                      Bank Wire
                    </span>
                  </div>
                  <div
                    className={`w-4 h-4 rounded-full border-2 ${selectedPayment === "bank" ? "border-lime-500 bg-lime-500" : "border-zinc-200"}`}
                  ></div>
                </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center border ${selectedPayment === "paypal" ? "bg-white text-blue-500 border-blue-100 shadow-sm" : "bg-blue-50 text-blue-500 border-blue-100"}`}
                    >
                      <Wallet size={16} />
                    </div>
                    <span className={`font-bold text-sm ${selectedPayment === "paypal" ? "text-zinc-900" : "text-zinc-600"}`}>
                      PayPal / Skrill
                    </span>
                  </div>
                  <div
                    className={`w-4 h-4 rounded-full border-2 ${selectedPayment === "paypal" ? "border-lime-500 bg-lime-500" : "border-zinc-200"}`}
                  ></div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-zinc-900 rounded-[2.5rem] p-8 md:p-12 border border-zinc-800 text-white relative overflow-hidden group flex flex-col cursor-default">
            ...
            <div className="relative z-10 mt-4 flex-1">
              ...
              <p className="text-zinc-400 leading-relaxed mb-8 max-w-sm">
                Monitor your campaigns in real time with detailed reports, conversion tracking, and audience insights to
            ...
            <div className="mt-4 bg-zinc-800 rounded-2xl p-6 border border-zinc-700 relative overflow-hidden h-48 w-full flex items-end">
              ...
              <svg
                className="w-full h-full absolute inset-0 z-10 p-4"
                viewBox="0 0 400 150"
              ...
              >
                ...
                <path
                  d="M0,150 L0,100 C50,80 100,120 150,90 C200,60 250,100 300,50 C350,0 400,40 400,40 L400,150 Z"
                  fill="url(#chartGradient)"
                />
                <path
                  d="M0,100 C50,80 100,120 150,90 C200,60 250,100 300,50 C350,0 400,40 400,40"
                  fill="none"
                  stroke="#bef264"
                  strokeWidth="3"
                ...
                <g transform="translate(260, 10)">
                  ...
                  <text x="40" y="20" fontSize="12" fontWeight="bold" fill="#18181b" textAnchor="middle">
                    $12.4k
                  </text>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            ...
            <div
              key={i}
              className="bg-zinc-50 rounded-3xl p-8 border border-zinc-100 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-default"
            >
        ...
        <div className="mt-16 text-center">
          <div className="bg-lime-50 border border-lime-100 rounded-2xl p-8 max-w-3xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
            ...
            <button
              ...
              className="bg-lime-400 text-zinc-900 px-6 py-3 rounded-full font-bold hover:bg-lime-300 transition-colors shadow-lg shadow-lime-200"
            >
              Join Network
            </button>
          </div>
