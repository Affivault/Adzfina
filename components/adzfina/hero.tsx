  ...
  return (
    <section className="relative pt-32 pb-20 overflow-hidden bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        {/* Headlines */}
        <h1 className="text-5xl md:text-7xl font-bold text-zinc-900 tracking-tight mb-6 leading-[1.1] max-w-4xl mx-auto text-balance">
          Join. Convert. <span className="text-zinc-400">Profit.</span>
          <br className="hidden md:block" />
          <span className="relative inline-block mt-2">
            Powered by AdzFina.
            <svg
              className="absolute w-full h-3 -bottom-1 left-0 text-lime-300 z-[-1]"
              viewBox="0 0 100 10"
        ...
        <p className="text-xl text-zinc-500 mb-10 max-w-2xl mx-auto leading-relaxed text-pretty">
          Maximize your earnings with top-paying forex and crypto offers. Get exclusive deals, fast payouts, and
          industry-leading support - all in one powerful network.
        </p>

        <div className="flex items-center justify-center gap-4 mb-20">
          <button
            onClick={openAuthModal}
            className="bg-zinc-900 text-white px-8 py-4 rounded-full font-semibold hover:bg-zinc-800 transition-all shadow-xl hover:shadow-2xl hover:-translate-y-1"
          >
            Get Started
          </button>
          <button
            onClick={() => scrollToSection("features")}
            className="bg-white text-zinc-900 border border-zinc-200 px-8 py-4 rounded-full font-semibold hover:bg-zinc-50 transition-all flex items-center gap-2"
          >
            ...
            <div className="bg-zinc-50 rounded-[1.5rem] overflow-hidden flex min-h-[700px] text-left">
              ...
              <div className="hidden md:flex w-64 bg-white border-r border-zinc-100 flex-col justify-between p-6">
                ...
                <div className="border-t border-zinc-100 pt-6">
                  <div
                    className="bg-lime-50 p-4 rounded-xl mb-6 border border-lime-100 cursor-help"
                    title="Contact Support"
                  >
                    <p className="text-xs font-bold text-lime-800 mb-1">Account Manager</p>
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-lime-200 flex items-center justify-center text-lime-700 text-xs font-bold">
                        SJ
                      </div>
                  ...
                  <div
                    onClick={() => showToast("Signed out successfully (Demo)", "info")}
                    className="flex items-center gap-2 text-zinc-400 hover:text-zinc-600 cursor-pointer text-xs font-medium"
                  >
                ...
                <header className="sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-zinc-200 px-8 py-4 flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="hidden sm:flex items-center gap-2 bg-white border border-zinc-200 px-3 py-1.5 rounded-full text-xs text-zinc-500 shadow-sm focus-within:border-zinc-400 focus-within:ring-1 focus-within:ring-zinc-100 transition-all">
                      ...
                      />
                      <span className="ml-2 text-zinc-300 hidden md:inline">{"Cmd+K"}</span>
                    </div>
                    <div
                      onClick={() => showToast("No new notifications", "info")}
                      className="w-8 h-8 rounded-full bg-white border border-zinc-200 flex items-center justify-center text-zinc-500 relative cursor-pointer hover:bg-zinc-50 transition-colors"
                    >
                      <Bell size={16} />
                ...
                <div className="p-8 space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div className="bg-zinc-900 p-6 rounded-3xl shadow-xl shadow-zinc-200 text-white relative overflow-hidden group hover:scale-[1.02] transition-transform cursor-default">
                      ...
                      <p className="text-zinc-400 text-xs font-medium uppercase tracking-wider mb-1 relative z-10">
                        Total Revenue
                      </p>
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border border-zinc-100 hover:shadow-lg transition-shadow">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <button
                          ...
                          className="text-[10px] font-bold text-zinc-600 border border-zinc-200 px-2 py-1 rounded hover:bg-zinc-900 hover:text-white transition-all"
                        >
                          Withdraw
                        </button>
                      </div>
                  ...
                  <div className="grid lg:grid-cols-3 gap-8">
                      ...
                      <div className="h-64 w-full relative group cursor-crosshair">
                        ...
                        <svg viewBox="0 0 800 300" className="w-full h-full overflow-visible">
                          ...
                          <g transform="translate(280, 110)">
                            ...
                            <text x="30" y="20" fill="white" fontSize="12" fontWeight="bold" textAnchor="middle">
                              $4,250
                            </text>
                          </g>
                        </svg>
                    ...
                    <div className="bg-white p-6 rounded-[2rem] border border-zinc-100 shadow-sm flex flex-col">
                      ...
                      <div className="space-y-4 flex-1">
                        {[
                          { name: "CapitalFX Pro", payout: "$850 CPA", tag: "Forex" },
                          { name: "CryptoVault", payout: "$400 CPA", tag: "Crypto" },
                          { name: "PrimeForex", payout: "$700 CPA", tag: "Forex" },
                        ].map((offer, i) => (
                          <div
                            key={i}
                            onClick={() => handleOfferClick(offer.name)}
                            className="flex items-center justify-between p-4 bg-zinc-50 rounded-2xl hover:bg-zinc-100 transition-colors cursor-pointer group"
                          >
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-xl bg-white border border-zinc-200 flex items-center justify-center text-xs font-bold text-zinc-500 group-hover:border-lime-300 transition-colors">
                                {offer.name.substring(0, 2).toUpperCase()}
                              </div>
                              <div>
                                <p className="font-semibold text-sm text-zinc-900">{offer.name}</p>
                                <p className="text-xs text-zinc-400">{offer.tag}</p>
                              </div>
                            </div>
                            <span className="text-sm font-bold text-lime-600 bg-lime-50 px-2 py-1 rounded-full">
                              {offer.payout}
                            </span>
                          </div>
                        ))}
