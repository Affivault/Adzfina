    ...
    <nav className="fixed top-0 w-full z-50 py-4 px-4 sm:px-6">
      <div
        className={`max-w-6xl mx-auto transition-all duration-300 ${
          isScrolled
            ? "bg-white/80 backdrop-blur-xl shadow-sm border border-zinc-100 rounded-full py-3 px-6"
            : "bg-transparent py-2"
        }`}
      >
        <div className="flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-1">
            <div className="bg-zinc-100/50 rounded-full px-2 py-1 flex items-center gap-1">
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-zinc-600 hover:text-zinc-900 hover:bg-white hover:shadow-sm rounded-full transition-all"
              >
                Features
              </button>
              <button
          ...
          <div className="md:hidden flex items-center">
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-zinc-900 p-2">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-20 left-4 right-4 bg-white rounded-3xl border border-zinc-100 shadow-xl p-6 z-40">
          <div className="flex flex-col space-y-4">
            <button
              onClick={() => handleNavClick("features")}
              className="text-lg font-medium text-zinc-900 p-2 hover:bg-zinc-50 rounded-xl text-left"
            >
              Features
            </button>
            <button
            ...
            <button
              ...
              className="bg-zinc-900 text-white px-6 py-4 rounded-xl text-lg font-medium w-full"
            >
              Start for free
            </button>
          </div>
